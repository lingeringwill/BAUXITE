...
# License code: LEUQNRUDIRCKIGFR
    ...
    def main():
        ...
        def redraw_window():
            ...
            if lost:
                lost_label = lost_font.render("Recycling job is done for today!", 1, (255,255,255))
                screen.blit(lost_label, (s_width/2 - lost_label.get_width()/2, 350))
                # Load and draw the "return to home menu" button
                home_button = pygame.transform.scale(pygame.image.load("homeButton.png").convert_alpha(), (500, 500))
                screen.blit(home_button, (s_width/2 - home_button.get_width()/2, 300))
                time.sleep(2)
                pygame.quit()
